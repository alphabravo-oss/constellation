...
func admissionCriteriaCatalog() []admissionCriterionOption {
	return []admissionCriterionOption{
		...
		{Key: "host_ipc", Label: "Uses host IPC", ValueType: "none", Help: "Deny pods that share the host IPC namespace."},
		{Key: "allow_privilege_escalation", Label: "Allows privilege escalation", ValueType: "none", Help: "Deny containers that set allowPrivilegeEscalation=true."},
		{Key: "image_no_os", Label: "Image has no OS", ValueType: "none", Help: "Deny images with no OS layer (scratch/distroless base)."},
		{Key: "require_resource_limits", Label: "Missing CPU/memory limits", ValueType: "none", Help: "Deny containers that omit CPU or memory requests or limits."},
		...
		{Key: "max_medium_cves", Label: "Medium CVEs over", ValueType: "int", Placeholder: "20", Help: "Deny if the image has more than N medium CVEs."},
		{Key: "max_critical_with_fix_cves", Label: "Fixable critical CVEs over", ValueType: "int", Placeholder: "0", Help: "Deny if the image has more than N critical CVEs that have a fix available."},
		{Key: "max_high_with_fix_cves", Label: "Fixable high CVEs over", ValueType: "int", Placeholder: "3", Help: "Deny if the image has more than N high CVEs that have a fix available."},
